...
const SuperfastWireless: React.FC = () => {
  ...
  const intervalTime = 2000; // Change this value to adjust the speed (milliseconds)

  useEffect(() => {
    const rows: NodeListOf<HTMLLIElement> | any =
      uiRef.current?.querySelectorAll('ul li');

    let currentProgress = 0;
    const totalSteps = rows ? rows.length : 1;

    const updateProgress = () => {
      setProgress(prevProgress => {
        let newProgress = prevProgress + 1;
        if (newProgress >= totalSteps) newProgress = 0;
        return newProgress;
      });

      if (rows) {
        rows.forEach((row: any, index: number) => {
          let progressValue = index === currentProgress ? 1 : 0;
          row.style.setProperty('--progress', progressValue.toString());
        });
      }

      currentProgress = (currentProgress + 1) % totalSteps;
    };
